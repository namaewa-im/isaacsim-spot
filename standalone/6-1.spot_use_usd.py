...
class SpotCameraExample:
    def __init__(self):
        ...
        # 캡처를 위한 설정
        self.capture_interval = 10  # 매 10 스텝마다 캡처 (1초 = 500 스텝, 0.02초마다 이미지 캡쳐, 초당 50장 이미지 캡처)
        self.step_count = 0
        self.capture_count = 0
        
        # 현재 날짜로 폴더 생성
        current_date = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        self.output_dir = f"./spot_data/{current_date}"
        os.makedirs(self.output_dir, exist_ok=True)
        
    def load_custom_environment(self):
        """사용자 정의 USD 환경 로드"""
        usd_path = "/home/tree/isaacsim/standalone_examples/isaacsim_env/2025-08-07env.usd"
        if os.path.exists(usd_path):
            print(f"USD 환경 파일 로딩 중: {usd_path}")
            # USD 파일 로드
            omni.usd.get_context().open_stage(usd_path)
            print("USD 환경 파일 로딩 완료")
            return True
        else:
            print(f"USD 파일을 찾을 수 없음: {usd_path}")
            return False

    def setup_scene(self):
        """씬 설정"""
        # 사용자 USD 환경 로드 시도
        if not self.load_custom_environment():
            print("기본 ground plane 사용")
            # 기본 ground plane 생성 (USD 파일이 없는 경우)
        ...
        self.spot = SpotFlatTerrainPolicy(
            ...
            name="Spot",
            position=np.array([0, 0, 2.0]),  # z를 1.0에서 2.0으로 변경 (1m 더 위로)
        )
    ...
    def main(self):
        """메인 실행 함수"""
        print("Spot 로봇 body 카메라 부착 예제 시작 (사용자 USD 환경)")
        print("키보드 컨트롤:")
        ...
        # USD 파일을 먼저 로드 (World 생성 전에)
        self.load_custom_environment()
